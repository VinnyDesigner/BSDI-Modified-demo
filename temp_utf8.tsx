...
export default function DataAccessRequests1() {
  ...
  const dataAccessCompletedRequests = [
    ...
    ...applyRbacFilter(RAW_dataAccessPendingRequests).map(r => ({...r, status: 'Pending', requestedBy: r.requester, requestedDate: r.date, approvedBy: '├óÔé¼ÔÇØ', approvedDate: '├óÔé¼ÔÇØ'}))
  ];
  ...
  const dataDownloadCompletedRequests = [
    ...
    ...applyRbacFilter(RAW_dataDownloadPendingRequests).map(r => ({...r, status: 'Pending', requestedBy: r.requestor, requestedDate: r.date, approvedBy: '├óÔé¼ÔÇØ', approvedDate: '├óÔé¼ÔÇØ'}))
  ];
  const metadataCompletedRequests = [
    ...
    ...applyRbacFilter(RAW_metadataPendingRequests).map(r => ({...r, status: 'Pending', requestedBy: r.requestor, requestedDate: r.date, approvedBy: '├óÔé¼ÔÇØ', approvedDate: '├óÔé¼ÔÇØ'}))
  ];
  const appUsersCompletedRequests = [
    ...
    ...applyRbacFilter(RAW_appUsersPendingRequests).map(r => ({...r, status: 'Pending', requestedBy: r.requestedBy || "Jawaher Rashed", requestedDate: r.requestedDate, approvedBy: '├óÔé¼ÔÇØ', approvedDate: r.requestedDate || '├óÔé¼ÔÇØ'}))
  ];
                                    ...
                                    <td className="whitespace-nowrap">
                                      <Tooltip>
                                        ...
                                        <TooltipContent className="bg-gray-800 text-white text-xs">
                                          {group.fileName} ├óÔé¼ÔÇØ {group.fileSize}
                                        </TooltipContent>
                                      </Tooltip>
                                    </td>
...
<div className="mt-4">
  ...
  return searchMatch && statusMatch;
            ...
            <tr key={request.id}>
              ...
              <td>{request.service}</td>
              <td>{request.organization || "├óÔé¼ÔÇØ"}</td>
              <td>{request.approvedBy || "├óÔé¼ÔÇØ"}</td>
              <td>{request.reqDate}</td>
              <td>{request.appDate || "├óÔé¼ÔÇØ"}</td>
              <td>{request.approvedBy || "├óÔé¼ÔÇØ"}</td>
            ...
            <TabsContent value="spatial-permission">
              <Tabs defaultValue="spatial-pending">
                ...
                <div className="flex items-center justify-between border-b border-[#E5E7EB] mb-4 pr-1">
                  ...
                  {/* Search / date filter ├óÔé¼ÔÇØ shown per sub-tab */}
                        ...
                        <TooltipProvider delayDuration={100}>
                          {filteredSpatialPending.map((request) => (
                            <tr key={request.id}>
                              ...
                              <td className="whitespace-nowrap">{request.boundary || "├óÔé¼ÔÇØ"}</td>
                              <td className="font-medium whitespace-nowrap text-[#374151]">
                                ...
                                <tr key={request.id}>
                                  ...
                                  <td className="whitespace-nowrap">
                                    <a href={"mailto:" + request.email} className="text-[#3D72A2] hover:underline text-[13px] flex items-center">
                                      {request.email} <span className="ml-1 text-[10px]">├óÔÇáÔÇö</span>
                                    </a>
                                  </td>
                                  <td className="sticky-col-actions">
                                  {isOrgAdmin ? (
...
<TabsContent value="app-users-completed" className="mt-0 !m-0 p-0 border-0 flex-1 flex justify-end" tabIndex={-1}>
  ...
  return searchMatch && statusMatch;
            ...
            {/* ├óÔÇØÔé¼├óÔÇØÔé¼├óÔÇØÔé¼ Reject Dialog ├óÔÇØÔé¼├óÔÇØÔé¼├óÔÇØÔé¼ */}
            ...
            {/* ├óÔÇØÔé¼├óÔÇØÔé¼├óÔÇØÔé¼ Document Viewer Dialog ├óÔÇØÔé¼├óÔÇØÔé¼├óÔÇØÔé¼ */}
            ...
            {/* ├óÔÇØÔé¼├óÔÇØÔé¼├óÔÇØÔé¼ Roles Popover Dialog ├óÔÇØÔé¼├óÔÇØÔé¼├óÔÇØÔé¼ */}
          ...
          <div className="flex-1 flex flex-col px-6 pt-6 overflow-hidden">
            ...
            <div className="grid grid-cols-3 gap-3 mb-5">
              <div className="bg-[#F9FAFB] rounded-[12px] p-3 border border-[#F3F4F6]">
                ...
                <div className="text-[13px] font-bold text-[#111827]">{previewingRequest?.id || "├óÔé¼ÔÇØ"}</div>
              </div>
              <div className="bg-[#F9FAFB] rounded-[12px] p-3 border border-[#F3F4F6]">
                <div className="flex items-center gap-2 mb-1.5 text-[#6B7280]">
