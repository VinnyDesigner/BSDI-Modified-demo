const fs = require('fs');
const filePath = 'd:/Projects/BSDI/BSDI-Modified-demo/src/app/pages/modules/Departments.tsx';
let content = fs.readFileSync(filePath, 'utf-8');

const target = /      \{\/\* Users Sheet \*\/\}[\s\S]*?<\/Sheet>/;
const replacement = `      {/* Users Sheet */}
                ...
                {departmentUsers[selectedDeptForSheet.id].map((user) => (
                  <div key={user.id} className="bg-[#FFFFFF] border border-[#F1F5F9] rounded-[12px] p-[12px_14px] flex items-center justify-between transition-all hover:border-[#E5E7EB] hover:bg-[#F9FAFB] hover:shadow-sm h-[64px]">
                    <div className="flex items-center gap-[12px] flex-1 min-w-0">
                      <div className="w-[36px] h-[36px] md:w-[40px] md:h-[40px] rounded-full bg-[#FEE2E2] flex items-center justify-center font-semibold text-[#EF4444] text-[14px] shrink-0 border border-[#FEE2E2]">
                    ...
                    <div className="shrink-0 ml-[8px] flex items-center justify-end">
                      <span className="bg-[#EEF2FF] text-[#4F46E5] text-[11px] md:text-[12px] font-medium px-[10px] py-[4px] rounded-[999px] whitespace-nowrap inline-flex items-center justify-center uppercase tracking-[0.02em] min-w-[70px]">
                        {user.role}
                      </span>
                    </div>
                  </div>
