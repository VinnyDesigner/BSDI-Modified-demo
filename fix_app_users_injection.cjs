...
// The TabsTrigger and MockData injected might already exist from last run. Let's make sure TabsContent gets injected!
...
if (metadataStart !== -1) {
    ...
    if (!content.includes('<TabsContent value="app-users">')) {
         ...
         console.log('⚠️ App Users Tab already exists.');
    }
} else {
    console.log('❌ Could not find Metadata tab to insert after.');
}
